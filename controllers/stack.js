...
exports.getAllStacks = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Cannot get Stacks. Please try again.' });
  }
  return true;
};
...
exports.getOneStack = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Cannot get this Stack. Please try again." });
  }
  return true;
};
...
exports.updateOneStack = async (req, res) => {
    ...
    try {
      ...
      const updateStack = await Stack.update({ ...stackObject }, { where: { id: req.params.StackId } });
      if (updateStack) {
...
exports.deleteOneStack = async (req, res) => {
  try {
    ...
    const deleteStack = await Stack.destroy({ where: { id: req.params.StackId } });
    if (deleteStack) {
